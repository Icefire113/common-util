...
use crate::errors::UtilReadError;

pub fn read_f32_le<R: Read>(reader: &mut R) -> Result<f32, UtilReadError> {
    let mut buff = [0u8; size_of::<f32>()];
    reader.read_exact(&mut buff)?;
    Ok(f32::from_le_bytes(buff))
}

pub fn read_f64_le<R: Read>(reader: &mut R) -> Result<f64, UtilReadError> {
...
pub fn read_n_bytes<R: Read>(reader: &mut R, n: usize) -> Result<Vec<u8>, UtilReadError> {
...
pub fn read_n_bytes_const<R: Read, const N: usize>(
    reader: &mut R,
...
pub fn read_n_bytes_to_buff<R: Read>(
    reader: &mut R,
    buff: &mut Vec<u8>,
    n: usize,
...
/// Reads `n` bytes from the reader into a caller supplied buffer
pub fn read_n_bytes_to_slice<R: Read>(
    reader: &mut R,
    buff: &mut [u8],
    n: usize,
) -> Result<(), UtilReadError> {
...
pub fn read_u8_le<R: Read>(reader: &mut R) -> Result<u8, UtilReadError> {
...
pub fn read_u16_le<R: Read>(reader: &mut R) -> Result<u16, UtilReadError> {
...
pub fn read_u32_le<R: Read>(reader: &mut R) -> Result<u32, UtilReadError> {
...
/// Reads a u32 from the reader
pub fn read_u32_be<R: Read>(reader: &mut R) -> Result<u32, UtilReadError> {
    let mut buff = [0u8; size_of::<u32>()];
    reader.read_exact(&mut buff)?;
...
pub fn read_i32_le<R: Read>(reader: &mut R) -> Result<i32, UtilReadError> {
...
pub fn read_u64_le<R: Read>(reader: &mut R) -> Result<u64, UtilReadError> {
...
/// Reads a u64 from the reader
pub fn read_u64_be<R: Read>(reader: &mut R) -> Result<u64, UtilReadError> {
    let mut buff = [0u8; size_of::<u64>()];
    reader.read_exact(&mut buff)?;
    Ok(u64::from_be_bytes(buff))
}

/// Reads a C style string from the reader, checks to ensure UTF8 validity
pub fn read_string<R: BufRead>(reader: &mut R) -> Result<String, UtilReadError> {
    let mut buff = vec![];
...
pub fn read_until_bytes<R: Read + Seek>(
    reader: &mut R,
    pattern: [u8; 2],
...
pub fn skip<R: Read + Seek>(reader: &mut R, n: u64) -> io::Result<()> {
